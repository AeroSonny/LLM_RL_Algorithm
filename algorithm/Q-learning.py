...
class QLearning:
    def __init__(self, 
        ...
        # q_table은 상태 수 x 행동 수 크기의 2차원 배열로, 모든 값이 0으로 초기화됨
        # 상태수와 행동수를 곱한 행렬로 나타내는 것은 각 상태에서 가능한 모든 행동에 대한 Q-값을 저장하기 위함
        # 예를 들어, 상태가 16개이고 행동이 4개라면, q_table은 16x4 크기의 배열이 되어 각 상태에서 4가지 행동에 대한 Q-값을 저장할 수 있음
        # 이렇게 하면 에이전트가 특정 상태에 있을 때 어떤 행동을 선택해야 하는지에 대한 정보를 효율적으로 관리할 수 있음
        # (0,0) 위치는 상태 0에서 행동 0에 대한 Q-값을 나타냄 에를 들어, q_table[0, 0]은 상태 0에서 행동 0을 선택했을 때의 예상 보상을 의미함
        # Q-테이블의 각 행은 특정 상태에 해당하며, 각 열은 해당 상태에서 가능한 행동에 해당함
        # 양수이면 해당 행동이 유리하다는 것을 의미하고, 음수이면 불리하다는 것을 의미함 예를들어, (0,0) = 0.5 라면 상태 0에서 행동 0을 선택했을 때 예상 보상이 0.5임을 의미하며,
